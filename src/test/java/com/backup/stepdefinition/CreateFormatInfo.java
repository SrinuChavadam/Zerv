...
public class CreateFormatInfo extends TestRunner{
	...
	@Given("^am a Format_Information Team member \"(.*?)\"$")
	...
	@When("^user provides a publicationID along with \"(.*?)\" and \"(.*?)\" and \"(.*?)\" values$")
	public void getResponse(String processedLookupId, String contentTypeLookupId, String availableLookupId) throws Throwable {
		    	    
		this.publicationId=api.genarateRandomNumber();
		String json=api.createRequestFormatInfo(processedLookupId, contentTypeLookupId, availableLookupId);
		 request.body(json.toString());
		 response = request.post(this.publicationId);	
	...
	@Then("^new record should be created in Format_Information table and validate aganist Database along with \"(.*?)\" and \"(.*?)\" and \"(.*?)\" values$")
