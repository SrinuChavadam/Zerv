...
public class CreateAcquisitionInformation extends TestRunner{
	...
	@Given("^am a Acquisition_Information Team member \"(.*?)\"$")
	...
	public void getResponse(String transmissionTypeLookupId, String receiptMethodLookupId,String categoryLookupId,String ftpTypeLookupId,String filePriorityLookupId,String contentSizeLookupId,String browserLookupId,String triggerLookupId) throws Throwable {
	  		
		
		 String query="select publication_id from available_format";
	      Set<String> publicationIDs = db.getDbListValues(query, "publication_id");
	         for (String value : publicationIDs)
	         {    
	             publicationId=value;
	        String query1="select count(*) as count from acquisition_info where publication_id='"+publicationId+ "'";
	        int count=db.getRowCount(query1);
	        if(count==0)
	        {
	           	        	
	        	String createAcquisitionInfoRequest = api.createAcquisitionInfoRequest(transmissionTypeLookupId, receiptMethodLookupId, categoryLookupId, ftpTypeLookupId, filePriorityLookupId, contentSizeLookupId, browserLookupId, triggerLookupId);	
	     		response=request.body(createAcquisitionInfoRequest.toString()).post(publicationId);
	...
	@Then("^new record should be created in Acquisition_Information table and validate aganist Database along with\"(.*?)\" and \"(.*?)\" and \"(.*?)\" and \"(.*?)\" and \"(.*?)\" and \"(.*?)\" and \"(.*?)\" and \"(.*?)\"values$")
	public void validateDatabase(String transmissionTypeLookupId , String receiptMethodLookupId ,String categoryLookupId, String ftpTypeLookupId ,String filePriorityLookupId ,String contentSizeLookupId ,String browserLookupId ,String triggerLookupId) throws Throwable {
	  ...
	  SoftAssert sa=new SoftAssert();
		 {
		  sa.assertEquals(transmissionTypeLookupId, transmissionTypeLookupIdFromDb,"Available lookup Id Mismatched from database");
		  sa.assertEquals(receiptMethodLookupId, receiptMethodLookupIdFromDb,"contentTypeLookupId Mismatched from database");
		  ...
		  sa.assertEquals(filePriorityLookupId, filePriorityLookupIdFromDb,"processedLookupId Mismatched from database");
		  sa.assertEquals(contentSizeLookupId, contentSizeLookupIdFromDb,"processedLookupId Mismatched from database");
