...
public class EditContentPackaging extends TestRunner{
	...
	@Given("^am a technicalEvaluator_editContentPackage \"(.*?)\"$")
	public void am_a_technicalEvaluator_editContentPackage(String endpoint) throws Throwable {
	...
	public void user_provides_content_packaging_Information_for_publicationId(String zipName, String zipNameAccess, String zipContent, String volume,
		...
		String query = "select publication_id from content_packaging_info";
		...
		for (String value : publicationIDs) {
			...
			if (count > 0) {
				...
				response = request.body(createContentPackageRequest.toString()).put(publicationId);
	...
	public void the_user_should_get_a_success_response_editContentPackage() throws Throwable {
	...
	@Then("^data should be created in content_packaging_info,file_name_convention and folder_name_convention Tables with \"(.*?)\",\"(.*?)\",\"(.*?)\",\"(.*?)\",\"(.*?)\",\"(.*?)\",\"(.*?)\",\"(.*?)\",\"(.*?)\",\"(.*?)\",\"(.*?)\",\"(.*?)\",\"(.*?)\",\"(.*?)\",\"(.*?)\",\"(.*?)\" for publicationId$")
	public void data_should_be_created_in_content_packaging_info_file_name_convention_and_folder_name_convention_Tables_with_for_publicationId(String zipName, String zipNameAccess, String zipContent, String volume,
			String formatLupId, String notes, String matching, String issue, String folderId,String folderCon,String folderName,String folderType, String folderStructure,String fileId,String fileName,String nameCon) throws Throwable {
	...
	public void the_user_should_get_a_error_response_editContentPackage() throws Throwable {
		int statusCode=response.getStatusCode();
		Reporter.addStepLog(Integer.toString(statusCode));
		Assert.assertTrue(statusCode == 400, "status code is not correct");
	}
   //Scenario-3
	@Given("^am not a technicalEvaluator_editContentPackage \"(.*?)\"$")
	public void am_not_a_technicalEvaluator_editContentPackage(String endpoint) throws Throwable {
	...
	public void the_user_should_get_Error_response_editContentPackage() throws Throwable {
		int statusCode=response.getStatusCode();
		Reporter.addStepLog(Integer.toString(statusCode));
